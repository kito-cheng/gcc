...
static const attribute_spec riscv_gnu_attributes[] =
{
  ...
  {"riscv_vls_cc", 0, 1, false, true, true, true, riscv_handle_rvv_vls_cc_attribute, NULL},
...
static const attribute_spec riscv_attributes[] =
{
  ...
  {"vls_cc", 0, 1, false, true, true, true, riscv_handle_rvv_vls_cc_attribute, NULL},
  /* This attribute is used to declare a new type, to appoint the exactly
     bits size of the type.  For example:
			       ...
			       unsigned abi_vlen = 0)
{
  ...
  return riscv_flatten_aggregate_field (type, fields, 0, 0,
					ignore_zero_width_bit_field_p, vls_p, abi_vlen);
}
...
void
riscv_init_cumulative_args (CUMULATIVE_ARGS *cum, const_tree fntype, rtx, tree, int)
{
...
static rtx
riscv_pass_vls_in_vr (struct riscv_arg_info *info,
		      ...
		      bool return_p)
{
  ...
  rtx reg = riscv_get_vector_arg(info, cum, vla_mode, return_p);
...
riscv_pass_aggregate_in_vr (struct riscv_arg_info *info,
			      ...
			      bool return_p)
{
  // TODO: Extend riscv_flatten_aggregate_argument to handle up to 4 fields.
  riscv_aggregate_field fields[8];
  unsigned int abi_vlen = riscv_get_cc_abi_vlen (cum->variant_cc);
  int n = riscv_flatten_aggregate_argument (type, fields, true, /* vls_p */true, abi_vlen);

  if (n == -1)
    return NULL_RTX;

  // Check all field has same size;
  unsigned int mode_size = GET_MODE_SIZE (TYPE_MODE (fields[0].type)).to_constant ();
  for (int i = 1; i < n; i++)
    if (GET_MODE_SIZE (TYPE_MODE (fields[i].type)).to_constant () != mode_size)
      return NULL_RTX; /* Return NULL_RTX if we cannot find a suitable reg.  */


  // Check total size is <= abi_vlen * 8.
  unsigned int total_size = 0;
  int i;

  for (i = 0; i < n; i++)
    total_size += (GET_MODE_SIZE (TYPE_MODE (fields[i].type)).to_constant ());

  if (total_size > abi_vlen * 8)
    return NULL_RTX; /* Return NULL_RTX if we cannot find a suitable reg.  */

  // Check there is enough register to pass the aggregate.
  // pass in ref if not enough registers.
  // TODO: Count by how many vec arg reg and then check with abi_vlen.

  // Backup cum->used_vrs since we will defer the update until the
  // riscv_function_arg_advance
  CUMULATIVE_ARGS local_cum;
  memcpy (&local_cum, cum,
	  sizeof (local_cum));
  ...
  for (i = 0; i < n; i++)
    {
       ...
       machine_mode vla_mode = riscv_get_vls_constainer_type (mode, abi_vlen);
       // Cheating riscv_get_vector_arg to that is always not return_p
       rtx reg = riscv_get_vector_arg(info, &local_cum, vla_mode, /*return_p*/ false);
	 ...
	 return NULL_RTX;
       ...
       expr_list[i] = gen_rtx_EXPR_LIST (VOIDmode,
					 reg,
					 GEN_INT (fields[i].offset));
			    ...
			    const function_arg_info &arg)
{
  ...
  if ((info.num_vrs > 0 || info.num_mrs > 0) && cum->variant_cc != RISCV_CC_V && !riscv_vls_cc_p (cum->variant_cc))
    {
...
rtx
riscv_libcall_value (machine_mode mode, const_rtx fun)
{
  struct riscv_arg_info info;
  CUMULATIVE_ARGS args;
