...
/* { dg-final { scan-rtl-dump {\(set \(reg/v:V4SF \d+ \[ float_vec \]\)[[:space:]]+\(reg:V4SF \d+ v9 \[ float_vec \]\)\)} "expand" } } */
// Check argument 3 register assignment
/* { dg-final { scan-rtl-dump {\(set \(reg/v:V2DF \d+ \[ double_vec \]\)[[:space:]]+\(reg:V2DF \d+ v10 \[ double_vec \]\)\)} "expand" } } */
