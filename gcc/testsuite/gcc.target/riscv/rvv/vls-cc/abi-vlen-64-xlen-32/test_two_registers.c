...
/* { dg-final { scan-rtl-dump {\(set \(reg/v:V8SI \d+ \[ vec \]\)[[:space:]]+\(reg:V8SI \d+ v8 \[ vec \]\)\)} "expand" } } */
// Check argument 2 register assignment
/* { dg-final { scan-rtl-dump {\(set \(reg/v:V8SI \d+ \[ vec2 \]\)[[:space:]]+\(reg:V8SI \d+ v12 \[ vec2 \]\)\)} "expand" } } */
// Check return value passed in vector register
/* { dg-final { scan-rtl-dump {\(set \(reg/i:V8SI \d+ v8\)[[:space:]]+\(reg:V8SI \d+ \[ <retval>.*\]\)\)} "expand" } } */
