...
/* { dg-final { scan-rtl-dump {\(set \(reg/v:V8HI \d+ \[ short_vec \]\)[[:space:]]+\(reg:V8HI \d+ v9 \[ short_vec \]\)\)} "expand" } } */
// Check argument 3 register assignment
/* { dg-final { scan-rtl-dump {\(set \(reg/v:V4SI \d+ \[ int_vec \]\)[[:space:]]+\(reg:V4SI \d+ v10 \[ int_vec \]\)\)} "expand" } } */
// Check argument 4 register assignment
/* { dg-final { scan-rtl-dump {\(set \(reg/v:V2DI \d+ \[ long_vec \]\)[[:space:]]+\(reg:V2DI \d+ v11 \[ long_vec \]\)\)} "expand" } } */
